...
- Safety accents: restrained orange, reserved for small renderer-generated hazard/status lamps rather than source-material panel fills.
...
| `structure-reactor` | Two Space Kit rocket assemblies at X `-0.74/+0.74`, each built from `rocket_baseA`, `rocket_sidesA`, `rocket_fuelA`, and `rocket_topA` |
...
Space Kit OBJ materials are solid-color `Kd` materials with no texture dependency. They map by material name into warm-white, graphite, and cool-steel levels; source hue must never promote a large surface to orange. `rock`, `rockDark`, and `metalRed` are neutralized with the same name-based hierarchy. Modular Space Kit `gate-lasers` and `room-large` preserve `Models/OBJ format/Textures/colormap.png`. Each reactor rocket uses base/sides/fuel/top scales `0.85/0.80/0.72/0.72` and Y offsets `0/0.72/1.40/2.02`; the X separation prevents the two normalized fuel bodies from intersecting.
...
Turret collision height and the procedural fallback barrel mount remain `1900` world units. Atlas-backed turrets use an independent visual `weaponMountHeight` of `1120`, derived from all fourteen committed body bounds and verified with the real `project()`/`worldSpriteDrawRect()` depth difference so the barrel overlaps every body slightly.
...
Automated verification covers source/license provenance, deterministic double rendering, atlas geometry, alpha correctness, canonical opaque bounds, runtime world dimensions, fallback behavior, yaw selection, drone direction cues, a maximum 20% orange coverage, a maximum 5% red/purple coverage, the turret mount connection, and 24-pixel gap padding on every edge.
