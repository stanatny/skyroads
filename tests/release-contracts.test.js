...
test('V1.3.0 metadata agrees across the static game and macOS bundle', () => {
  ...
  assert.deepEqual(version, {
    semver: '1.3.0', display: 'V1.3', accessible: '1.3', tag: 'v1.3.0',
  });
  assert.equal(Object.isFrozen(version), true);
  assert.match(html, /<meta name="application-version" content="1\.3\.0">/);
  assert.equal(plistString(plist, 'CFBundleShortVersionString'), packageJson.version);
  assert.equal(plistString(plist, 'CFBundleVersion'), '5');
});
...
test('release tag validation accepts only v plus package semver', () => {
  ...
  assert.equal(expectedReleaseTag('1.3.0'), 'v1.3.0');
  assert.equal(validateReleaseTag('v1.3.0', '1.3.0'), true);
  assert.equal(validateReleaseTag('V1.3', '1.3.0'), false);
  assert.equal(validateReleaseTag('v1.3', '1.3.0'), false);
  assert.equal(spawnSync(process.execPath, [script, 'v1.3.0']).status, 0);
  assert.notEqual(spawnSync(process.execPath, [script, 'V1.3']).status, 0);
  assert.notEqual(spawnSync(process.execPath, [script, 'v1.3']).status, 0);
});
