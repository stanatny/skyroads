...
## Track Five Shipping States

Track these independently:

1. the reviewed change is merged;
2. production deployment serves the intended merge;
3. the semantic tag points to that verified commit;
4. the Release exists for the tag;
5. the downloadable artifact or store/package build is verified.

Do not say “released” when only merge or deployment is complete. Avoid changing a stable README link to a Release that does not exist yet unless the project explicitly accepts that temporary gap.
...
4. Open a cache-busted canonical URL and verify commit/version diagnostics, visible UI, controls, gameplay, localization, and required assets. Then verify the ordinary canonical URL after its cache window.
...
Record the branch, PR, merge SHA, deployment identity, cache-busted and ordinary canonical URL evidence, verification time, tag target, release URL, artifact checksum, and any remaining manual evidence. Prefer retrying an idempotent failed stage over rewriting history.
